...
function f3() {
    ...
    let boi = document.querySelector(".out-3").innerHTML = a3.five + " " + a3.odd;
    return boi;
}
...
function f4() {
    let out = document.querySelector(".out-4").innerHTML;
    for(let key in a4){
        out += key + " " + a4[key] + '<br>';
    }
    return out;
...
function f6() {
    let inp1 = document.querySelector(".i-61").value;
    let inp2 = document.querySelector(".i-62").value;
...
function f7() {
    let keyz = document.querySelector(".i-7").value;
    for(let key in a7){
        if(a7.hasOwnProperty(keyz)){
...
function f8() {
    let smth = "x";
    for(let key in a8) {
        if(a8.hasOwnProperty(smth)){
            document.querySelector(".out-8").innerHTML = a8[smth];
        } else{
...
function f9() {
    let keyz = +document.querySelector(".i-9").value;
    for (let key in a9) {
        if (a9[key] === keyz) {
...
document.querySelector('.b-10').onclick = () => {
    ...
    document.querySelector('.out-10').innerHTML = f10(a10, 443);
};
...
function f11() {
    let result = document.querySelector(".i-11").value;
    delete a11[result];
    for(let key in a11) {
        document.querySelector(".out-11").innerHTML += "<br>" + key + ' ' + a11[key] + "<br>";
    }
}
...
function f12() {
let result = +document.querySelector(".i-12").value;
    for(let key in a12) {
        if(result == a12[key]) {
            delete a12[key]
...
function f15() {
    for(let key in a15) {
       let smth = a15[key].join(" ")
       document.querySelector(".out-15").innerHTML += smth + " ";
...
function f18() {
    let value = document.querySelector(".i-18").value;
        for(let key in a18) {
            if(a18.hasOwnProperty(value)) {
                let smth = a18[value].join(" ")
            document.querySelector(".out-18").innerHTML = smth;
            } else {
                document.querySelector(".out-18").innerHTML = "empty";
            }
        }
}
...
let a19 = {
    "red": ['Akademmistechko', 'Nyvky', 'Universytet', 'Lisova'],
    "blue": ['Minska', 'Obolon', 'Pochaina', 'qweq'],
    "green": ['Syrets', 'Zoloti Vorota', 'Klovska', 'Vidubichi']
}
...
function f19() {
    ...
    for (let key in a19) {
        for (let i = 0; i < a19.red.length; i++) {
            if (value == a19.red[i]) {
                document.querySelector(".out-19").innerHTML = "red";
            } else if (value == a19.blue[i]) {
                document.querySelector(".out-19").innerHTML = "blue";
            } else if (value == a19.green[i]) {
                document.querySelector(".out-19").innerHTML = "green";
            } else {
...
function f20() {
    for(let key in a20) {
        for(let i = 0; i < a20[key].length; i++) {
            if(a20[key][i][1] === 2) {
                document.querySelector(".out-20").textContent += a20[key][i][0] + " ";
            }
        }
    }
