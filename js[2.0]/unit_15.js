...
const f7 = () => {
   ...
   if(setInput.size == input.length && input.length > 6) {
...
let s8 = new Set([1000, 1, 2, 3, 4, 5, 3, 4, 7, 9, 5, 7, 8, 9, 23, 45, 5, 2, 4, 5, 3, 24, 5, 2, 4, 56, 4, 3, 2, 335, 2, 23, 41, 3, 4, 1, 1, 4, 2, 2, 4, 5, 24, 5, 3, 22, 56]);
let ar8 = [];
...
const f9 = our_set => {
    n_our_set = Array.from(our_set);
    return n_our_set.join(" ");
}
...
const f10 = (out_set, elem) => {
    new_set = Array.from(out_set);
    document.querySelector(elem).innerHTML = new_set.join(" ");
}
