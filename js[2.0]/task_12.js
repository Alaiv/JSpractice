...
function f8() {
    ...
    for (let i = 0; i < a8.length; i++) {
        for (let j = 0; j < a8.length; j++) {
            if (a8[i][j] % 2 === 1) out += a8[i][j] + ' ';
        }
    }
...
function f14() {
    for(let i = 0; i < a14.length; i++){
       document.querySelector(".out-14").innerHTML += a14[i].length + ' ';
    }
}
...
function f15() {
    ...
    for(let i = 0; i < a15.length; i++){
        if(a14[i].length > biggest){
            biggest = a15[i].length;
        }
    }
    document.querySelector(".out-15").innerHTML = biggest;
}
